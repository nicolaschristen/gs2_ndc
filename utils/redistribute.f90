...
!AJ copy functionality
  ...
  subroutine c_redist_32_old_copy(r, from_here, to_here)
    ...
    !JTP
    !write(*,*) "Entering c_redist_32_old_copy"
    !write(*,*) "from_here"
    !write(*,*) from_here
    !write(*,*) "to_here"
    !write(*,*) to_here
...
!AJ point to be considered.    
  ...
  ! MRH these routines are copies of R_redist_42, the dimensionality of from here increased to 6
   subroutine r_redist_62 (r, from_here, to_here)
    use job_manage, only: time_message
    use mp, only: iproc, nproc, send, receive, proc0
    type (redist_type), intent (in out) :: r
    ...
    real, dimension (r%from_low(1):, &
                        ...
                        r%from_low(4):, &
                        r%from_low(5):, &
                        r%from_low(6):), intent (in) :: from_here
    ...
    if (proc0.and.(.not.using_measure_scatter)) call time_message(.false.,time_redist,' Redistribute')
  end subroutine r_redist_62

  subroutine r_redist_62_inv (r, from_here, to_here)
    use job_manage, only: time_message
    use mp, only: iproc, nproc, send, receive, proc0
    type (redist_type), intent (in out) :: r
    ...
    real, dimension (r%from_low(1):, &
                        ...
                        r%from_low(4):, &
                        r%from_low(5):, &
                        r%from_low(6):), intent (in out) :: to_here
    ...
    if (proc0.and.(.not.using_measure_scatter)) call time_message(.false.,time_redist,' Redistribute') 
  end subroutine r_redist_62_inv
  
    subroutine r_redist_26 (r, from_here, to_here)
    use job_manage, only: time_message
    use mp, only: iproc, nproc, send, receive, proc0
    type (redist_type), intent (in out) :: r
    ...
    if (proc0.and.(.not.using_measure_scatter)) call time_message(.false.,time_redist,' Redistribute')
  end subroutine r_redist_26

  subroutine r_redist_26_inv (r, from_here, to_here)
    use job_manage, only: time_message
    use mp, only: iproc, nproc, send, receive, proc0
    type (redist_type), intent (in out) :: r
    ...
    if (proc0.and.(.not.using_measure_scatter)) call time_message(.false.,time_redist,' Redistribute') 
  end subroutine r_redist_26_inv
  ! MRH
...
!       endif
  ...
  subroutine c_redist_36_mpi_copy_nonblock(r, from_here, to_here, ntgrid)
    ...
    integer :: i, ip, nsend, nrecv, count, ipfrom, msgsize
...
!             call nbsend (send_buff(:,:,1:r%from(ip)%nn,count),ip,iproc,send_hand(count))
  ...
  subroutine c_redist_36_inv (r, from_here, to_here, ntgrid)
    use job_manage, only: time_message
    use mp, only: iproc, proc0
    integer, intent(in) :: ntgrid
    type (redist_type), intent (in out) :: r
...
!       else
          call c_redist_36_inv_mpi_copy_nonblock(r,from_here,to_here,ntgrid)
!       endif
    else
       call c_redist_36_inv_mpi_copy(r, from_here, to_here,ntgrid)
    endif

    if (proc0.and.(.not.using_measure_scatter)) call time_message(.false.,time_redist,' Redistribute')
  end subroutine c_redist_36_inv

  subroutine c_redist_36_inv_mpi_copy(r, from_here, to_here, ntgrid)
  ...
  subroutine c_redist_36_inv_mpi_copy_nonblock(r, from_here, to_here, ntgrid)
