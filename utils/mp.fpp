...
! CMR: defined MPIINC for machines where need to include mpif.h
  ...
  integer, target :: comm_all, comm_group, comm_multigs2
...
!  subroutine init_mp
  subroutine init_mp (comm_in, multigs2)
# ifdef MPI
...
# endif
    ...
    logical, intent (in), optional :: multigs2 ! MRH
# ifdef MPI
    integer :: ierror!, rank
#ifdef OPENMP
    integer :: provided
    ...
    if (.not. init) then
       ...
       if(provided .ne. MPI_THREAD_FUNNELED) then
          write(*,*) 'Problem with MPI_INIT_THREAD, stopping'
      stop
       end if
    end if
#else
...
!MAB>
    if (present(comm_in)) then
       if (present(multigs2)) then
         comm_multigs2= comm_in
       else
         comm_all = comm_in
    ...
    if(present(multigs2)) then
      ...
      call mpi_comm_size (comm_multigs2, mulntot_proc, ierror)
      call mpi_comm_rank (comm_multigs2, mulproc, ierror)
        
      mulproc0 = mulproc == 0 ! MRH
...
# ifdef MPI
  ...
  subroutine nb_allgatherv_complex_array_1to3_sub(arr,count,out,recvcnts,displs,sub_comm,request)
    !<DD>A subroutine to do a non-blocking allgatherv operation, sending recvcnts(iproc)
    !data from the iproc'th processor to all others starting at arr(start).
    implicit none
    complex, dimension(:), intent(in) :: arr  !The data to gather
    integer, intent(in) :: count !How much data to gather, <=SIZE(arr)
...
# ifdef MPI
    integer :: ierror
    call mpi_bcast (char, size(char), MPI_CHARACTER, 0, mp_comm, ierror)
# endif
...
# endif
  ...
  subroutine nbsend_complex_2d_array(z,dest,tag,handle)
    !<AJ>Routine for nonblocking send of z to dest. Use
    !tag to label message and return handle for later checking.
    implicit none
    complex, dimension(:,:), intent(in) :: z
    integer, intent(in) :: dest
