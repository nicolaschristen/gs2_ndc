    ...
    if(.not. remap_plot) then
        ...
        do iglo = g_lo%llim_proc, g_lo%ulim_proc
           ...
           do ig = -ntgrid, ntgrid
              ...
              aj0(ig,iglo) = j0(arg)
! CM    R 17/1/06: BEWARE, j1 returns and aj1 stores J_1(x)/x (NOT J_1(x)), 
              aj1(ig,iglo) = j1(arg)
           end do
        end do
...
!    if (alloc) then
    if (.not. allocated(g)) then
       ...
       if (explicit_terms_present) then
#ifndef SHMEM
          allocate (gexp_1(-ntgrid:ntgrid,2,g_lo%llim_proc:g_lo%ulim_alloc))
          write(*,*) "dist_fn::allocate_arrays, allocated gexp_1,2,3 correctly"
#else
          call shm_alloc(gexp_1, (/ -ntgrid, ntgrid, 1, 2, &
               g_lo%llim_proc, g_lo%ulim_alloc/))
#endif
          ...
          write(*,*) "dist_fn::allocate_arrays, did NOT allocate gexp_1,2,3 correctly"
       end if
#endif
       if (boundary_option_switch == boundary_option_linked) then
...
!    alloc = .false.
  ...
  subroutine timeadv (phi, apar, bpar, phinew, aparnew, bparnew, istep, mode,trigger_aminv_opt, trigger_interp_opt) ! NDCTESTtime: added timers
    ...
    ! NDCTESTremap_plot
    use gs2_time, only: code_dt
    use gs2_transforms, only: transform2, update_flowshear_phase_fac
    ...
    if(remap_plot_nl) then
        ...
        if(remap_plot_nl_analytic) then
            ...
            ! First, update phase_fac to phase_fac(t[it])
            call update_flowshear_phase_fac(g_exb)

            !! Write g(code_time)
            ! transform g to real space
            gb = 0.
            gexp_3 = g ! can overwrite gexp_3, it will be thrown away anyway
            call transform2(gexp_3, gb)
...
!                killing array allocations!
    ...
    use nonlinear_terms, only: nonlinear_mode_switch, nonlinear_mode_on !NDCgexp
                      ...
                      do it = 1, ntheta0 + jump(ik)                        
                         ...
                         if (idx_local(g_lo, to_iglo) .and. idx_local(g_lo, from_iglo)) then
                            ...
                            if(remap_gexp) then
                                do isgn=1, 2
                                   mptag = 1
                                   call send (gexp_1(:, isgn, from_iglo), proc_id (g_lo, to_iglo),mptag)
                                   mptag = 2
                                   call send (gexp_2(:, isgn, from_iglo), proc_id (g_lo, to_iglo),mptag)
                                enddo
                            end if
                         else if (idx_local(g_lo, to_iglo)) then
                            do isgn=1, 2
                            ...
                            if(remap_gexp) then
                                do isgn=1, 2
                                   mptag = 1
                                   call receive (gexp_1(:, isgn, to_iglo), proc_id (g_lo, from_iglo),mptag)
                                   mptag = 2
                                   call receive (gexp_2(:, isgn, to_iglo), proc_id (g_lo, from_iglo),mptag)
                                enddo
                            end if
                         endif
                      enddo
      ...
      subroutine add_explicit (g1, g2, g3, phi, apar, bpar, istep, bd)
          ...
          if (istep /= istep_last) then
              ...
              if(nonlinear_mode_switch == nonlinear_mode_on) then
                  ...
                  ! NDCTESTnl
                  ! update_flowshera_phase_fac is already called at beginning of
                  ! timeadv for remap_nl cases.
