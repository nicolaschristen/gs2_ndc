...
! </doc>
  ...
  public :: gryfx
  public :: box_get_flowshear_flags
  ...
  ! Private variables. Public versions are hosted in kt_grids --NDC 11/2017
...
! If n0 and rhostar_box defined, set ky(1) using toroidal mode number.
  ...
  subroutine box_get_flowshear_flags(explicit_flowshear_public, implicit_flowshear_public, &
      ...
      logical, intent(inout) :: remap_gexp_public ! NDCgexp
      ...
      remap_gexp_public = remap_gexp ! NDCgexp
...
contains
  ...
  subroutine init_kt_grids
    ...
    ! Only use the continuous flow-shear implementation in box mode --NDC 11/2017
    if(proc0) then
    ...
    call broadcast (remap_gexp) ! NDCgexp
