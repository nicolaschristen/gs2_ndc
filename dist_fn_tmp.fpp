...
contains
  ...
  subroutine init_dist_fn_arrays
    ...
    use species, only: nspec
    use kt_grids, only: naky, ntheta0, explicit_flowshear, mixed_flowshear
  ...
  subroutine init_dist_fn_level_3
    ...
    if (debug) write(6,*) "init_dist_fn: compute_wdrift"
    call init_wdrift
...
!CMR
  ...
  ! calc_vdrift_x -> code_dt * (0.5*vperp**2*gbdrift + vpar**2*cvdrift)
  ! calc_vdrift_y -> code_dt * (0.5*vperp**2*gbdrift0 + vpar**2*cvdrift0)
  ! wdrift_func = 0.5*ky * (theta0*calc_vdrift_x + calc_vdrift_y)
  function calc_vdrift_y (ig, il, ie)
    use theta_grid, only: bmag, gbdrift, cvdrift
    ...
    calc_vdrift_y = code_dt * ( &
                    0.5*al(il)*energy(ie)*bmag(ig)*gbdrift(ig) + &
                    energy(ie)*(1.0-al(il)*bmag(ig))*cvdrift(ig) )
  
  end function calc_vdrift_y
  
  function calc_vdrift_x (ig, il, ie)
    use theta_grid, only: bmag, gbdrift0, cvdrift0
    use le_grids, only: energy, al
    use gs2_time, only: code_dt
    implicit none
    real :: calc_vdrift_x
    ...
    calc_vdrift_x = code_dt * ( &
                    0.5*al(il)*energy(ie)*bmag(ig)*gbdrift0(ig) + &
                    energy(ie)*(1.0-al(il)*bmag(ig))*cvdrift0(ig) )      
  ...
  ! NDC 11/2017
  ! Splitting wcoriolis_func in the same way as wdrift_func above
  function calc_vcoriolis_x(ig,il,ie,is)
    use theta_grid, only: bmag, cdrift0
    use le_grids, only: energy, al
    use gs2_time, only: code_dt
    ...
    real :: calc_vcoriolis_x
    integer, intent(in) :: ig,il,ie,is

    calc_vcoriolis_x = mach*sqrt(max(energy(ie)*(1.0-al(il)*bmag(ig)),0.0)) &
            * code_dt/spec(is)%stm * cdrift0(ig)

  end function calc_vcoriolis_x

  function calc_vcoriolis_y(ig,il,ie,is)
    use theta_grid, only: bmag, cdrift
    use le_grids, only: energy, al
    use gs2_time, only: code_dt
    use species, only: spec
    ...
    real :: calc_vcoriolis_y
    integer, intent(in) :: ig,il,ie,is

    calc_vcoriolis_y = mach*sqrt(max(energy(ie)*(1.0-al(il)*bmag(ig)),0.0)) &
            * code_dt/spec(is)%stm * cdrift(ig)
...
! CMR 17/1/06: BEWARE, j1 returns and aj1 stores J_1(x)/x (NOT J_1(x)), 
          ...
          if (aky(ik) /= 0.0) then
              ...
              do it = 1, ntheta0
                  ...
                  kperp2_tdep%old(:,it,ik) = kperp2(:,it,ik) + &
                      (my_kx_shift(ik)+aky(ik)*g_exb*g_exbfac*gdt*tunits(ik))* &
                      (my_kx_shift(ik)+aky(ik)*g_exb*g_exbfac*gdt*tunits(ik))*gds22/(shat*shat) + &
  ...
  subroutine init_invert_rhs
    ...
    real :: wdttp, vp, bd
    real :: wdold, wdnew, wdttpold, wdttpnew
#ifdef LOWFLOW
    complex :: wd
#else
    real :: wd
    ...
    if (.not.allocated(a)) then
        ...
        ! Depending on the flow-shear implementation, different arrays become time-dependent
        ! and need to be updated at each time-step:
        !  -- explicit or mixed: a and b
        !  -- implicit: a, b, ainv and r
        ! NDC 02/2018
        if(implicit_flowshear) then
            a = 0.
            b = 0.
...
# endif
             ...
             if(implicit_flowshear) then
                 ainv(ig,isgn,iglo) &
                      = 1.0/(1.0 + bd &
                      + (1.0-fexp(is))*spec(is)%tz*(zi*wdnew*(1.0+bd) + 2.0*vp))
                 r(ig,isgn,iglo) &
                      = (1.0 - bd &
                      + (1.0-fexp(is))*spec(is)%tz*(zi*wdnew*(1.0-bd) - 2.0*vp)) &
                      *ainv(ig,isgn,iglo)
                 a(ig,isgn,iglo) &
                      = 1.0 + bd &
                      + fexp(is)*spec(is)%tz*(-zi*wdold*(1.0+bd) - 2.0*vp)
                 b(ig,isgn,iglo) &
                      = 1.0 - bd &
                      + fexp(is)*spec(is)%tz*(-zi*wdold*(1.0-bd) + 2.0*vp)
             else
...
if( isgn.eq.1 )then
                ...
                if (il >= ittp(ig) .and. .not. forbid(ig, il)) then
                    if(implicit_flowshear) then
                        ainv(ig,isgn,iglo) = 1.0/(1.0 + zi*(1.0-fexp(is))*spec(is)%tz*wdttpnew)
                        a(ig,isgn,iglo) = 1.0 - zi*fexp(is)*spec(is)%tz*wdttpold
                    else
...
! CMR, 5/10/2010: multiply timestep by tunits(ik) for wstar_units=.true. 
    if ( box .or. shat .eq. 0.0 ) then
       do ik=1, naky
          ...
          kx_shift_old(ik) = kx_shift_old(ik) - jump(ik)*dkx ! now take jumps into account for kx_shift at previous time
       end do
       !do ig = -ntgrid,ntgrid
...
! now the distribution functions
  ...
  subroutine get_source_term &
    ...
    use dist_fn_arrays, only: aj0, aj1, vperp2, vpar, vpac, g, ittp, &
        aj0_tdep, wdrift_tdep, wdriftttp_tdep
    use theta_grid, only: ntgrid
    ...
    integer :: ig
    complex, dimension (-ntgrid:ntgrid) :: apargavg
    ! NDCTESTtime
    !complex, dimension (-ntgrid:ntgrid) :: phigavg, phigavgnew
    complex, dimension(-ntgrid:ntgrid) :: phij0_old, phij0_new, phij0_told, phij0_tnew
    real :: fexpfac, fimpfac
    ! endNDCTESTtime
    real :: c0, c1, c2
...
!
    ...
    ! NDCTESTtime: for readability
    phij0_old = phi(:,it,ik)*aj0(:,iglo)*fphi
    phij0_new = phinew(:,it,ik)*aj0(:,iglo)*fphi
    if(explicit_flowshear .or. mixed_flowshear .or. implicit_flowshear) then
        phij0_told = phi(:,it,ik)*aj0_tdep%old(:,iglo)*fphi
        if(implicit_flowshear) then
            phij0_tnew = phinew(:,it,ik)*aj0_tdep%new(:,iglo)*fphi
        end if
    end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    case (source_option_full)
       if (il <= lmax) then
          ...
          if (proc0 .and. istep > 0) call time_message(.false.,timer_aminv_setsource,' Init_rhs') ! NDCTESTtime2
          if (proc0 .and. istep > 0) call time_message(.false.,timer_interp_setsource,' Init_rhs') ! NDCTESTtime2
          call set_source
          !if (proc0 .and. trigger_timer_interp2) call time_message(.false.,timer_interp_setsource,' Init_rhs') ! NDCTESTtime
          !if (proc0 .and. trigger_timer_aminv2) call time_message(.false.,timer_aminv_setsource,' Init_rhs') ! NDCTESTtime
          if (proc0 .and. istep > 0) call time_message(.false.,timer_aminv_setsource,' Init_rhs') ! NDCTESTtime2
...
! NDC 11/2017
    ...
    if (source_option_switch == source_option_full .or. &
       ...
       if (nlambda > ng2 .and. isgn == 2) then
          do ig = -ntgrid, ntgrid-1
             if (il < ittp(ig)) cycle
             if(implicit_flowshear) then ! NDCQUEST: what about mixed & explicit ?
                 source(ig) &
                      = g(ig,2,iglo)*a(ig,2,iglo) &
#ifdef LOWFLOW
                      - anon(ie)*zi*(wdttpfac(ig,it,ik,ie,is,2)*hneoc(ig,2,iglo))*(fexpfac*phij0_old(ig)+fimpfac*phij0_new(ig)) &
                      + zi*wstar(ik,ie,is)*hneoc(ig,2,iglo)*(fexpfac*phij0_old(ig)+fimpfac*phij0_new(ig))
#else
                      - anon(ie)*zi*(wdriftttp_tdep%new(ig,it,ik,ie,is,2))*fimpfac*phij0_tnew(ig) &
                      - anon(ie)*zi*(wdriftttp_tdep%old(ig,it,ik,ie,is,2))*fexpfac*phij0_told(ig) &
                      + zi*wstar(ik,ie,is)*(fexpfac*phij0_told(ig)+fimpfac*phij0_tnew(ig)) ! NDCQUEST: wstar used only tdep_old
#endif             
             elseif(explicit_flowshear .or. mixed_flowshear) then ! NDCQUEST: how to add new explicit terms here ?
                 source(ig) &
                      = g(ig,2,iglo)*a(ig,2,iglo) &
#ifdef LOWFLOW
                      - anon(ie)*zi*(wdttpfac(ig,it,ik,ie,is,2)*hneoc(ig,2,iglo))*(fexpfac*phij0_old(ig)+fimpfac*phij0_new(ig)) &
                      + zi*wstar(ik,ie,is)*hneoc(ig,2,iglo)*(fexpfac*phij0_old(ig)+fimpfac*phij0_new(ig))
#else
                      - anon(ie)*zi*(wdriftttp(ig,it,ik,ie,is,2))*(fexpfac*phij0_old(ig)+fimpfac*phij0_new(ig)) &
                      + zi*wstar(ik,ie,is)*(fexpfac*phij0_old(ig)+fimpfac*phij0_new(ig))
#endif           
             else
                 source(ig) &
                      = g(ig,2,iglo)*a(ig,2,iglo) &
...
! add in explicit terms 
  ...
  contains
    ...
    subroutine set_source
      ...
      use species, only: spec
      use theta_grid, only: shat, itor_over_B
      use dist_fn_arrays, only: kx_shift, kx_shift_old, wdrift_tdep
      implicit none
      complex :: apar_p, apar_m
      ! NDCTESTtime
      !complex :: phi_p, phi_m!, bpar_p !GGH added bpar_p
      !complex :: g_bd, j0phi_bd, j0phi_tdep_bd
      complex :: phi_p_old, phi_p_new, phi_p_told, phi_p_tnew
      complex :: phi_m_old, phi_m_new, phi_m_told, phi_m_tnew
      complex :: g_p_old
      ! endNDCTESTtime
...
!CMRend
      ...
      do ig = -ntgrid, ntgrid-1
         ...
         !end if
         ! NDCTESTtime
         phi_p_old = bdfac_p*phij0_old(ig+1) + bdfac_m*phij0_old(ig)
         phi_m_old = phij0_old(ig+1) - phij0_old(ig)
         phi_p_new = bdfac_p*phij0_new(ig+1) + bdfac_m*phij0_new(ig)
         phi_m_new = phij0_new(ig+1) - phij0_new(ig)
         if(explicit_flowshear .or. mixed_flowshear .or. implicit_flowshear) then
             phi_p_told = bdfac_p*phij0_told(ig+1) + bdfac_m*phij0_told(ig)
             phi_m_told = phij0_told(ig+1) - phij0_told(ig)
             g_p_old = bdfac_p*g(ig+1,isgn,iglo) + bdfac_m*g(ig,isgn,iglo)
             if(implicit_flowshear) then
                 phi_p_tnew = bdfac_p*phij0_tnew(ig+1) + bdfac_m*phij0_tnew(ig)
                 phi_m_tnew = phij0_tnew(ig+1) - phij0_tnew(ig)
             end if
         end if
...
!
         ...
         source(ig) = anon(ie)*(vparterm(ig,isgn,iglo)*(fexpfac*phi_m_old+fimpfac*phi_m_new) &
              -spec(is)%zstm*vpac(ig,isgn,iglo) &
              *((aj0(ig+1,iglo) + aj0(ig,iglo))*0.5*apar_m  &
              + D_res(it,ik)*apar_p) &
              -zi*wdfac(ig,isgn,iglo)*(fexpfac*phi_p_old + fimpfac*phi_p_new)) &
              + zi*(wstarfac(ig,isgn,iglo) &
              + vpac(ig,isgn,iglo)*code_dt*wunits(ik)*ufac(ie,is) &
              -2.0*omprimfac*vpac(ig,isgn,iglo)*code_dt*wunits(ik)*g_exb*itor_over_B(ig)/spec(is)%stm) &
              *(fexpfac*phi_p_old + fimpfac*phi_p_new - apar_p*spec(is)%stm*vpac(ig,isgn,iglo))
         ...
         if(implicit_flowshear) then
             ...
             source(ig) = anon(ie)*(-2.0*vpar(ig,isgn,iglo)*(fexpfac*phi_m_told+fimpfac*phi_m_tnew) &
                  -spec(is)%zstm*vpac(ig,isgn,iglo) &
                  *((aj0(ig+1,iglo) + aj0(ig,iglo))*0.5*apar_m  &
                  + D_res(it,ik)*apar_p) &
                  -zi*wdrift_tdep%old(ig,isgn,iglo)*fexpfac*phi_p_told &
                  -zi*wdrift_tdep%new(ig,isgn,iglo)*fimpfac*phi_p_tnew ) &
                  + zi*(wstar(ik,ie,is) &
                  + vpac(ig,isgn,iglo)*code_dt*wunits(ik)*ufac(ie,is) &
                  -2.0*omprimfac*vpac(ig,isgn,iglo)*code_dt*wunits(ik)*g_exb*itor_over_B(ig)/spec(is)%stm) &
                  *(fexpfac*phi_p_told + fimpfac*phi_p_tnew - apar_p*spec(is)%stm*vpac(ig,isgn,iglo))

         elseif(mixed_flowshear .or. explicit_flowshear) then
             ...
             source(ig) = anon(ie)*(-2.0*vpar(ig,isgn,iglo)*(fexpfac*phi_m_old + fimpfac*phi_m_new) &
                  -spec(is)%zstm*vpac(ig,isgn,iglo) &
                  *((aj0(ig+1,iglo) + aj0(ig,iglo))*0.5*apar_m  &
                  + D_res(it,ik)*apar_p) &
                  -zi*wdrift(ig,isgn,iglo)*(fexpfac*phi_p_old+fimpfac*phi_p_new)) &
                  + zi*(wstar(ik,ie,is) &
                  + vpac(ig,isgn,iglo)*code_dt*wunits(ik)*ufac(ie,is) &
                  -2.0*omprimfac*vpac(ig,isgn,iglo)*code_dt*wunits(ik)*g_exb*itor_over_B(ig)/spec(is)%stm) &
                  *(fexpfac*phi_p_old + fimpfac*phi_p_new - apar_p*spec(is)%stm*vpac(ig,isgn,iglo))
             ...
             source(ig) = source(ig) &
                  - zi * spec(is)%tz * vdrift_x(ig,isgn,iglo)/shat * kx_shift_old(ik) * 0.5*g_p_old &
                  - zi * ( vdrift_x(ig,isgn,iglo)/shat*kx_shift_old(ik) + &
                      2.*wdrift(ig,isgn,iglo) ) * 0.5*phi_p_told & ! NDCQUEST: clean up 2.*0.5
                  + 2.*zi * wdrift(ig,isgn,iglo) * 0.5*phi_p_old &
                  - 2.*vpar(ig,isgn,iglo) * ( phi_m_told - phi_m_old ) &
                  - 2.*code_dt*zi * aky(ik)/spec(is)%stm * omprimfac * itor_over_B(ig) * vpac(ig,isgn,iglo) * g_exb * &
                  0.5*(phi_p_told - phi_p_old) &
                  + 2.*zi * wstar(ik,ie,is) * 0.5*(phi_p_told - phi_p_old)
...
! add in nonlinear terms 
  ...
  subroutine add_explicit_terms(g1, g2, g3, phi, apar, bpar, istep, bd)
      ...
      if(explicit_terms_present .and. istep /= 0) then
      ...
      subroutine add_explicit (g1, g2, g3, phi, apar, bpar, istep, bd)
          use theta_grid, only: ntgrid
          use gs2_layouts, only: g_lo, ik_idx, it_idx
          use unit_tests, only: debug_message
          use nonlinear_terms, only: initializing, istep_last, gryfx_zonal, &
          ...
          if (istep /= istep_last) then
              ...
              ! If running with explicit terms, then compute those terms at grid points
              ! and store them in g1
...
!
          ...
          use nonlinear_terms, only: nl_forbid_force_zero
...
!
                      gtmp(ig,1,iglo) = 0.5*((1.+bd)*gtmp(ig+1,1,iglo) + (1.-bd)*gtmp(ig,1,iglo))
                      gtmp(ig,2,iglo) = 0.5*((1.-bd)*gtmp(ig+1,2,iglo) + (1.+bd)*gtmp(ig,2,iglo))
             ...
             if (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) then
                adjleft = anon(ie)*2.0*vperp2(-ntgrid,iglo)*aj1(-ntgrid,iglo) &
                     *bparnew(-ntgrid,it,ik)*fbpar &
                     + spec(is)%z*anon(ie)*phinew(-ntgrid,it,ik)*aj0_tdep%new(-ntgrid,iglo) & ! NDCQUEST: should this (and below) be t-dep ?
                     /spec(is)%temp*fphi
             else
                adjleft = anon(ie)*2.0*vperp2(-ntgrid,iglo)*aj1(-ntgrid,iglo) &
                     *bparnew(-ntgrid,it,ik)*fbpar &
          ...
          if (r_links(ik,it) .eq. 0) then
             if (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) then
                adjright = anon(ie)*2.0*vperp2(ntgrid,iglo)*aj1(ntgrid,iglo) &
                     *bparnew(ntgrid,it,ik)*fbpar &
                     + spec(is)%z*anon(ie)*phinew(ntgrid,it,ik)*aj0_tdep%new(ntgrid,iglo) & ! NDCQUEST: same
                     /spec(is)%temp*fphi
             else
                adjright = anon(ie)*2.0*vperp2(ntgrid,iglo)*aj1(ntgrid,iglo) &
                     *bparnew(ntgrid,it,ik)*fbpar &
         ...
         if (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) then 
            adjl = anon(ie)*2.0*vperp2(ntgl,iglo)*aj1(ntgl,iglo) &
                 *bparnew(ntgl,it,ik)*fbpar &
                 + spec(is)%z*anon(ie)*phinew(ntgl,it,ik)*aj0_tdep%new(ntgl,iglo) & ! NDCQUEST: t-dep ?
                 /spec(is)%temp*fphi
            adjr = anon(ie)*2.0*vperp2(ntgr,iglo)*aj1(ntgr,iglo) &
                 *bparnew(ntgr,it,ik)*fbpar &
                 + spec(is)%z*anon(ie)*phinew(ntgr,it,ik)*aj0_tdep%new(ntgr,iglo) & ! NDCQUEST: same
                 /spec(is)%temp*fphi
         else
            adjl = anon(ie)*2.0*vperp2(ntgl,iglo)*aj1(ntgl,iglo) &
                 *bparnew(ntgl,it,ik)*fbpar &
...
!      
             ...
             if (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) then
                argl = spec(is)%bess_fac*spec(is)%smz*sqrt( &
                    energy(ie)*al(il)/bmag(-ntgrid)*kperp2_tdep%new(-ntgrid,itl,ik) ) ! NDCQUEST: should this be t-dep ?
                argr = spec(is)%bess_fac*spec(is)%smz*sqrt( &
                    energy(ie)*al(il)/bmag(ntgrid)*kperp2_tdep%new(ntgrid,itr,ik)) ! NDCQUEST: same
             else
       ...
       do iglo = g_lo%llim_proc, g_lo%ulim_proc
          do isgn = 1, 2
             do ig=-ntgrid, ntgrid
                ...
                elseif(present(expflow_opt)) then ! NDCQUEST: is this correct ?
                    select case(expflow_opt)
                    case(1)
                        ! NDCTESTfelix
                        g0(ig,isgn,iglo) = (aj0_tdep%old(ig,iglo) - aj0(ig,iglo)) * g(ig,isgn,iglo) &
                            + aj0(ig,iglo)*gnew(ig,isgn,iglo) ! NDCQUEST: Felix's method seems to blow up ?
                    case(2)
                        ! NDCTESTmichael for t-indep V/(1-gamma)
                        g0(ig,isgn,iglo) = aj0(ig,iglo)*g(ig,isgn,iglo)
                    case(3)
                        ! NDCTESTmichael for t-dep V/(1-gamma)
...
! return gnew to its initial state, the variable evolved in GS2
  ...
  subroutine getmoms_gryfx_dist (density_gryfx, upar_gryfx, tpar_gryfx, tperp_gryfx, qpar_gryfx, qperp_gryfx, phi_gryfx)
    ...
    g0 = (2.*sqrt(2.)*vpa*vpa*vpa - 3.*sqrt(2.)*vpa)*gnew
    call integrate_moment (g0, total)
    if(proc0) then
    do ig = -ntgrid, ntgrid-1
      iz = ig + ntgrid + 1
    ...
    do iglo = g_lo%llim_proc, g_lo%ulim_proc
       do isgn = 1, 2
          g0(:,isgn,iglo) = (vperp2(:,iglo)-1.)*gnew(:,isgn,iglo) 
       end do
    end do
    call integrate_moment (g0, total)
    if(proc0) then
  ...
  subroutine get_mom_glm (glm, l, m)
    ...
    use dist_fn_arrays, only: vpa, vperp2, gnew
    use gs2_layouts, only: g_lo
    use theta_grid, only: ntgrid
    ...
    do iglo = g_lo%llim_proc, g_lo%ulim_proc
      do isgn = 1, 2
        do ig = -ntgrid, ntgrid
          g0(ig,isgn,iglo) = hermite_prob(l, sqrt(2.0)*vpa(ig,isgn,iglo)) * &
                        laguerre_norm(m, vperp2(ig,iglo)) * gnew(ig,isgn,iglo) 
        end do
      end do
    end do
    call integrate_moment (g0, glm,  moment_to_allprocs, full_arr)
...
! adiabatic electrons 
  ...
  ! Updating time-dependent gamtot for cases with flow-shear.
  ! The optional argument shift_for_interp prevents gamtot_tdep%old from being changed and is only used
  ! to compute shifted versions of the field-solve matrix in fiedls_implicit::init_response_matrix. -- NDC 02/2018
...
!          end do
    ...
    if (fphi > epsilon(0.0)) then
       ...
       if(implicit_flowshear .or. mixed_flowshear) then
           ...
           !end do
       elseif(present(expflow_opt) .and. expflow_opt == 1) then ! NDCQUEST: is this correct ?
           fieldeq = antot + bpar*gamtot1 - gamtot_tdep%old*gridfac1*phi ! NDCQUEST: Felix's solution seems to blow up ?
       else
...
! in the following
  ...
  !AJ called from fields_gf_local or from fields_local when gf_lo_integrate is true.  Note, if this is called from
  !AJ fields_local and gf_lo_integrate is true then the calculation will be done locally but there is a function
  !AJ called after this has finished in fields_local that sends the data back (from gf_lo to g_lo layouts).
  !AJ When call with gf_lo_integrate = .true. this routine does a gather that converts the gnew array from g_lo
  !AJ data distribution to gf_lo data distribution and stores the result in gfarray.  With gf_lo_integrate = .false.
  !AJ gnew is used instead and this is in g_lo data distribution.
...
!AJ
    ...
    complex, dimension (-ntgrid:ntgrid,2,nspec,negrid,nlambda,gf_lo%llim_proc:gf_lo%ulim_alloc) :: gfarray
    integer :: ie, il, is
...
! Identical to get_flux except don't integrate over poloidal angle ! JRB
  subroutine get_flux_dist (fld, flx_dist, dnorm)
    use theta_grid, only: ntgrid, grho
    use kt_grids, only: ntheta0, aky, naky
    use le_grids, only: integrate_moment
    use species, only: nspec
...
!    use kt_grids, only: akx
    ...
    if (fphi > epsilon(0.0)) then
       ...
       call get_flux_dist (phi, pflux_dist, dnorm)

       do iglo = g_lo%llim_proc, g_lo%ulim_proc
          g0(:,:,iglo) = g0(:,:,iglo)*energy(ie_idx(g_lo,iglo))
       end do
       call get_flux_dist (phi, qflux_dist, dnorm)
       ...
       call get_flux_dist (phi, vflux_par_dist, dnorm)
       do iglo = g_lo%llim_proc, g_lo%ulim_proc
          it = it_idx(g_lo,iglo)
          ik = ik_idx(g_lo,iglo)
          is = is_idx(g_lo,iglo)
       ...
       call get_flux_dist (phi, vflux_perp_dist, dnorm)
    else
       pflux_dist = 0.
       vflux_par_dist = 0.
       vflux_perp_dist = 0.
