...
!  add public variables below for init_g
  ...
  ! NDCTEST_explicit_first & NDCTEST_explicit_second
  public :: expflowopt
  public :: expflowopt_none, expflowopt_second, expflowopt_antot_old, expflowopt_antot_new, &
...
! CMR 17/1/06: BEWARE, j1 returns and aj1 stores J_1(x)/x (NOT J_1(x)), 
  ...
  subroutine compute_a_b_r_ainv
      ...
      integer :: ig, ik, it, il, ie, is, isgn
      real :: vp, bd
      real, dimension(-ntgrid:ntgrid) :: wdttpold, wdttpnew
#ifdef LOWFLOW
      complex, dimension(-ntgrid:ntgrid) :: wdold, wdnew
#else
      ...
      ! Flow shear pointers
      !
      ! For var in {wdrift, wdriftttp, ainv, r},
      !
      ! var_ptr = var            (old/mixed/explicit flow shear algo)
      !         = var_tdep       (implicit algo & standard timeadv)
      !         = var_left/right (implicit algo & init interp aminv matrices)
      !
      ! NDC 02/2019
...
# ifdef LOWFLOW
              ! NDCQUEST: LOWFLOW not supported for new flow shear
              wdnew = wdrift(:,isgn,iglo) + wstar_neo(:,it,ik)*spec(is)%zt
              wdold = wdnew
...
!  end subroutine init_exb_shear
  ...
  subroutine exb_shear (g0, phi, apar, bpar, istep, field_local, undo_remap_opt)
...
!                killing array allocations!
    ...
    integer, intent(in) :: istep
    logical, intent(in), optional :: undo_remap_opt
...
! now the distribution functions
  ...
  subroutine get_source_term &
    ...
    use hyper, only: D_res
    use constants, only: zi
    use fields_arrays, only: phistar_old ! NDCTEST_explicit_first
    implicit none
    complex, dimension (-ntgrid:,:,:), intent (in) :: phi,    apar,    bpar
    complex, dimension (-ntgrid:,:,:), intent (in) :: phinew, aparnew, bparnew
    integer, intent (in) :: istep
...
!
    ...
    ! For var in {aj0, aj1},
    !
    ! var_ptr_src = var            (old/mixed/explicit flow shear algos)
    !             = var_tdep       (implicit algo & standard timeadv)
    !             = var_left/right (implicit algo & init interp aminv matrices)
    !
    ! For var in {wdrift, wdriftttp},
    !
    ! var_ptr = var            (old/mixed/explicit flow shear algos)
    !         = var_tdep       (implicit algo & standard timeadv)
    !         = var_left/right (implicit algo & init interp aminv matrices)
    !
    ! NDC 02/2019
    ...
    case (source_option_full)
       if (il <= lmax) then
          ...
          if(mixed_flowshear .or. explicit_flowshear) then
              ...
              call add_source_flowshear_mixed_and_explicit

              if(fapar>0.) then
                  call add_dapardt_flowshear_mixed_and_explicit
              end if

              if(explicit_flowshear .and. explicit_first) then
                  call add_source_flowshear_explicit_first
              end if
...
! add in explicit terms 
    ...
    end subroutine add_source_flowshear_mixed_and_explicit

    subroutine add_dapardt_flowshear_mixed_and_explicit
    ...
    end subroutine add_dapardt_flowshear_mixed_and_explicit

    subroutine add_source_flowshear_explicit_first
    ...
    end subroutine add_source_flowshear_explicit_first
      ...
      subroutine add_explicit (g1, g2, g3, phi, apar, bpar, istep, bd)
          ...
          if (istep /= istep_last) then
              ...
              if(nonlinear_mode_switch == nonlinear_mode_on) then
                  ...
                  if( flowshear_ptr_loc==flowshear_ptr_at_tdep .and. &
                      (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) .and. &
                      .not. apply_flowshear_nonlin ) then
                      call shift_ptr_to_std
                  else if( flowshear_ptr_loc==flowshear_ptr_at_std .and. &
                      .not. (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) .and. &
                      apply_flowshear_nonlin ) then
                      call shift_ptr_to_tdep
                  end if
                  ...
                  ! NDCTEST_nl_vs_lin: remove ptr shifting
                  if( flowshear_ptr_loc==flowshear_ptr_at_std &
                      .and. (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) &
                      .and. .not. apply_flowshear_nonlin) then
                      call shift_ptr_to_tdep
                  else if( flowshear_ptr_loc==flowshear_ptr_at_tdep .and. &
                      .not. (explicit_flowshear .or. implicit_flowshear .or. mixed_flowshear) &
                      .and. apply_flowshear_nonlin) then
                      call shift_ptr_to_std
                  end if
...
!  if speriod_flag = T, apply self-periodic bcs for passing and wfb.
    ...
    ! For var in {aj0, aj1},
    !
    ! var_ptr = var            (old flow shear algo)
    !         = var_tdep       (new algos & standard timeadv)
    !         = var_left/right (new algos & init interp aminv matrices)
    !
    ! NDC 02/2019
...
! region and at upper bounce point
    ...
    !
    ! var_ptr = var                   (old/mixed/explicit flow shear algos)
    !         = var [updated in time] (implicit algo & standard timeadv)
    !         = var_left/right        (implicit algo & init interp aminv matrices)
    !
    ! NDC 02/2019
...
!      
             ...
             !                = kperp2_tdep%new,   (new algos & standard timeadv)
             !                = kperp2_left/right, (new algos & init interp aminv matrices)
             !
             ! NDC 02/2019
...
!
  ...
  subroutine getan (antot, antota, antotp)
    use dist_fn_arrays, only: vpa, vperp2, gnew, &
        aj0_ptr, aj1_ptr, &
        g, aj0, aj1, aj0_tdep, aj1_tdep ! NDCTEST_explicit
    use kt_grids, only: kperp2, explicit_flowshear
    use species, only: nspec, spec
    use theta_grid, only: ntgrid
...
!use the various antots if we're not calculating/using the related field).
    ...
    !
    ! var_ptr = var            (old flow shear algo)
    !         = var_tdep       (mixed/implicit algos & standard timeadv)
    !         = var_left/right (mixed/implicit algos & init interp aminv matrices)
    !
    ! NDC 02/2019
    ...
    if (fphi > epsilon(0.0)) then
       ...
       if(explicit_flowshear) then
           ...
           case(expflowopt_second)
               ! NDCTEST_explicit_second
               do iglo = g_lo%llim_proc, g_lo%ulim_proc
                  do isgn = 1, 2
                     do ig=-ntgrid, ntgrid       
           ...
           case(expflowopt_antot_old)
               ! NDCTEST_explicit_first for t-indep V/(1-gamma), phistar[it]
               do iglo = g_lo%llim_proc, g_lo%ulim_proc
                  do isgn = 1, 2
                     do ig=-ntgrid, ntgrid       
           ...
           case(expflowopt_antot_tdep_old)
               ! NDCTEST_explicit_first for t-dep V/(1-gamma), phistar[it]
               do iglo = g_lo%llim_proc, g_lo%ulim_proc
                  do isgn = 1, 2
                     do ig=-ntgrid, ntgrid       
           ...
           case(expflowopt_antot_new)
               ! NDCTEST_explicit_first for t-indep V/(1-gamma), phistar[it+1]
               do iglo = g_lo%llim_proc, g_lo%ulim_proc
                  do isgn = 1, 2
                     do ig=-ntgrid, ntgrid       
                         g0(ig,isgn,iglo) = aj0(ig,iglo)*gnew(ig,isgn,iglo)
           ...
           case(expflowopt_antot_tdep_new)
               ! NDCTEST_explicit_first for t-dep V/(1-gamma), phistar[it+1]
               do iglo = g_lo%llim_proc, g_lo%ulim_proc
                  do isgn = 1, 2
                     do ig=-ntgrid, ntgrid       
           ...
           do iglo = g_lo%llim_proc, g_lo%ulim_proc
              do isgn = 1, 2
                 do ig=-ntgrid, ntgrid       
                    g0(ig,isgn,iglo) = aj0_ptr%new(ig,iglo)*gnew(ig,isgn,iglo)
                 end do
              end do
           end do
...
!    if (kfilter > epsilon(0.0)) call par_filter(antot)
       ! NDCQUEST: afilter not adapted to new flow shear algo.
    ...
    if (fapar > epsilon(0.0)) then
       ...
       ! NDCTEST_explicit: for explicit flow shear algo, this
       ! is wrong. EM is not adapted to explicit_flowshear yet.
       do iglo = g_lo%llim_proc, g_lo%ulim_proc
          do isgn = 1, 2
             do ig=-ntgrid, ntgrid
                g0(ig,isgn,iglo) = aj0_ptr%new(ig,iglo)*vpa(ig,isgn,iglo)*gnew(ig,isgn,iglo)
             end do
          end do
       end do
...
!    if (kfilter > epsilon(0.0)) call par_filter(antota)
    ...
    if (fbpar > epsilon(0.0)) then
       ...
       do iglo = g_lo%llim_proc, g_lo%ulim_proc
          do isgn = 1, 2
             do ig=-ntgrid, ntgrid
                g0(ig,isgn,iglo) = aj1_ptr%new(ig,iglo)*vperp2(ig,iglo)*gnew(ig,isgn,iglo)
             end do
          end do
       end do
...
! adiabatic electrons 
  ...
  subroutine getfieldeq1 (phi, apar, bpar, antot, antota, antotp, &
    ...
    use kt_grids, only: naky, ntheta0, aky, kperp2_ptr, &
        kperp2, explicit_flowshear ! NDCTEST_explicit
    use run_parameters, only: fphi, fapar, fbpar
    use run_parameters, only: beta, tite
    use species, only: spec, has_electron_species
    use dist_fn_arrays, only: gamtot_ptr, gamtot1_ptr, gamtot2_ptr, &
        gamtot_tdep, gamtot1_tdep, gamtot2_tdep ! NDCTEST_explicit
    implicit none
    complex, dimension (-ntgrid:,:,:), intent (in) :: phi, apar, bpar
...
!    if (first) allocate (fl_avg(ntheta0, naky))
    ...
    ! Flow shear pointers:
    !
    ! For var in {kperp2, gamtot, gamtot1, gamtot2},
    !
    ! var_ptr = var            (old flow shear algo)
    !         = var_tdep       (mixed/implicit algos & standard timeadv)
    !         = var_left/right (mixed/implicit algos & init interp aminv matrices)
    ...
    if (fphi > epsilon(0.0)) then
       if(explicit_flowshear) then
           ! NDCQUEST: bpar term is surely wrong since EM is not yet adapted to explicit flow shear algo.
           if(expflowopt==expflowopt_second) then
               fieldeq = antot + bpar*gamtot1 - gridfac1*phi * gamtot_tdep%old
           else
               fieldeq = antot + bpar*gamtot1 - gamtot*gridfac1*phi
           end if
       else
           fieldeq = antot + bpar*gamtot1_ptr%new - gamtot_ptr%new*gridfac1*phi
       end if
    ...
    if (fapar > epsilon(0.0)) then
       if(explicit_flowshear) then
           ! NDCTEST_explicit: probably wrong, EM is not yet adapted to explicit flow shear algo
           fieldeqa = antota - kperp2*gridfac1*apar
       else
           fieldeqa = antota - kperp2_ptr%new*gridfac1*apar
       end if
    end if
...
! in the following
    if (fbpar > epsilon(0.0)) then
       if(explicit_flowshear) then
           ...
           fieldeqp = (antotp+bpar*gamtot2_ptr%new+0.5*phi*gamtot1_ptr%new)*beta*apfac
       end if
...
!--------------------------------------------------------------------------- 
  subroutine calculate_flux_surface_average (fl_avg,antot)
    ...
    use dist_fn_arrays, only: gamtot_ptr, gamtot3_ptr
    implicit none
    ...
    ! Flow shear pointers:
    !
    ! gamtot3_ptr = gamtot3            (old flow shear algo)
    !             = gamtot3_tdep       (mixed/implicit algos & standard timeadv)
    !             = gamtot3_left/right (mixed/implicit algos & init interp aminv matrices)
    !
    ! NDC 02/2019
    ...
    if (.not. allocated(awgt)) then
       ...
       do ik = 1, naky
          ...
          do it = 1, ntheta0
             if(kwork_filter(it,ik)) cycle
             awgt(it,ik) = 1.0/sum(delthet*jacob*gamtot3_ptr%new(:,it,ik))
          end do
       end do
    endif
    ...
    do ik = 1, naky
       do it = 1, ntheta0
          if(kwork_filter(it,ik)) cycle
          fl_avg(it,ik) = tite*sum(delthet*jacob*antot(:,it,ik)/gamtot_ptr%new(:,it,ik))*awgt(it,ik)
       end do
    end do
